...
public sealed class MklBlasOperations : IBlasOperations
{
    public static readonly MklBlasOperations Default = new();
    ...
    {
        return (int)cblas_icamax(n, x, incX);
    }

    public unsafe int Icamin(int n, void* x, int incX)
    {
        return (int)cblas_icamin(n, x, incX);
    }
    ...
    {
        return (int)cblas_izamax(n, x, incX);
    }

    public unsafe int Izamin(int n, void* x, int incX)
    {
        return (int)cblas_izamin(n, x, incX);
    }
    ...
    {
        // MKL/CBLASにcblas_idmaxは存在しないため、Idamaxを流用（最大値のインデックス取得）
        return (int)cblas_idamax(n, x, incX);
    }

    public unsafe int Idmin(int n, double* x, int incX)
    {
        // MKL/CBLASにcblas_idminは存在しないため、最小値のインデックス取得は手動実装
        int minIdx = 0;
        if (n <= 0 || incX <= 0) return minIdx;
        double minVal = x[0];
        for (int i = 1, idx = incX; i < n; ++i, idx += incX)
        {
            if (x[idx] < minVal)
            {
                minVal = x[idx];
    ...
    {
        // MKL/CBLASにcblas_ismaxは存在しないため、Isamaxを流用（最大値のインデックス取得）
        return (int)cblas_isamax(n, x, incX);
    }

    public unsafe int Ismin(int n, float* x, int incX)
    {
        // MKL/CBLASにcblas_isminは存在しないため、最小値のインデックス取得は手動実装
        int minIdx = 0;
        if (n <= 0 || incX <= 0) return minIdx;
        float minVal = x[0];
        for (int i = 1, idx = incX; i < n; ++i, idx += incX)
